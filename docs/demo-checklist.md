...
Última validação Docker: `docker compose up -d --build`, `make docker-demo` e `make smoke` executaram com sucesso.
...
| B | Docker com `docker compose up -d --build`, `make docker-demo` e `make smoke` | Quando faltar `bitcoin-cli` local ou houver conflito de ambiente |
