...
export function ReorgLab({ onInspect, onGoToDashboard }: Props) {
  ...
  const txid = proof?.txid ?? (steps.find((s) => s.data?.txid)?.data?.txid as string | undefined)
  ...
  return (
    <div style={{ fontFamily: 'monospace', color: '#e5e7eb' }}>
      ...
      >
        ...
        {data?.error && <span style={{ fontSize: '11px', color: '#ef4444' }}>⚠ {data.error}</span>}
      </div>
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          {steps.length === 0 ? (
            <div style={{ color: '#374151', fontSize: '12px' }}>{t.reorg.noResults}</div>
          ) : (
      ...
      <LearnMore>{t.learn.reorg}</LearnMore>
    </div>
  )
}
...
function ProofCards({
  ...
  const cards = [
    {
      label: <Term term="TXID">{t.reorg.txid}</Term>,
      key: 'txid',
      value: proof.txid,
      mono: true,
      canInspect: true,
    ...
    {
      label: <Term term="Block hash">{t.reorg.originalBlock}</Term>,
      key: 'originalBlock',
      value: proof.original_block_hash ? proof.original_block_hash.slice(0, 20) + '…' : '—',
      mono: true,
    },
    {
      label: <Term term="Block height">{t.reorg.originalHeight}</Term>,
      key: 'originalHeight',
      value: proof.original_block_height ?? '—',
    },
    {
      label: <Term term="Confirmation">{t.reorg.confirmationsBefore}</Term>,
      key: 'confirmationsBefore',
      value: proof.confirmations_before_reorg,
    },
    {
      label: <Term term="Mempool">{t.reorg.mempoolAfterInvalidation}</Term>,
      key: 'mempoolAfterInvalidation',
      value: proof.mempool_status_after_invalidation,
    },
    {
      label: <Term term="Block hash">{t.reorg.finalBlock}</Term>,
      key: 'finalBlock',
      value: proof.final_block_hash ? proof.final_block_hash.slice(0, 20) + '…' : '—',
      mono: true,
    },
    {
      label: <Term term="Block height">{t.reorg.finalHeight}</Term>,
      key: 'finalHeight',
      value: proof.final_block_height ?? '—',
    },
    {
      label: <Term term="Confirmation">{t.reorg.finalConfirmations}</Term>,
      key: 'finalConfirmations',
      value: proof.final_confirmations,
    },
    {
      label: <Term term="Reorg">{t.reorg.chainRecovery}</Term>,
      key: 'chainRecovery',
      value: proof.mempool_status_after_recovery,
    },
    {
      label: t.reorg.reconsiderBlockCalled,
      key: 'reconsiderBlockCalled',
      value: proof.reconsider_block_called ? t.reorg.yes : t.reorg.no,
    },
  ]
