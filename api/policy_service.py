...
def _sub_ensure_wallet(scenario_id: str) -> bool:
    ...
    _set_step(scenario_id, "ensure_wallet", "running", f"Preparing wallet '{POLICY_WALLET}' and funds…")
    try:
        rpc = get_client()
        ...
        _set_step(scenario_id, "ensure_wallet", "error", "Wallet/funds setup failed.", error=str(exc))
        return False
...
def _sub_check_mempool(scenario_id: str, txid: str, step_id: str = "check_mempool") -> dict[str, Any] | None:
    ...
    try:
        ...
        _set_step(scenario_id, step_id, "error", "Transaction not found in mempool.", error=str(exc))
        return None
    ...
    try:
        ...
        if confirmations > 0:
            _set_step(
                ...
                technical={"txid": txid, "confirmations": confirmations, "blockhash": blockhash, "fee": fee},
                data={"confirmed": True, "confirmations": confirmations, "blockhash": blockhash},
            )
            return True
        else:
            _set_step(scenario_id, step_id, "error", "Not yet confirmed.", error=f"confirmations={confirmations}")
            return False
    except RPCError as exc:
...
def _run_normal_transaction(scenario_id: str) -> None:
    ...
    # send_normal_tx
    _set_step(scenario_id, "send_normal_tx", "running", "Sending normal transaction (default fee)…")
    try:
        w_rpc = _wallet_rpc()
        ...
        _set_step(scenario_id, "send_normal_tx", "error", "sendtoaddress failed.", error=str(exc))
        _finish_scenario(scenario_id, False)
        return
...
def _build_proof_normal(scenario_id: str) -> None:
    ...
    try:
        ...
        _set_step(
            scenario_id, "build_proof", "success",
            "Proof report ready.", technical=proof, data={"proof_ready": True},
        )
    except Exception as exc:
        _set_step(scenario_id, "build_proof", "error", "Proof assembly failed.", error=str(exc))
...
def _run_low_fee_transaction(scenario_id: str) -> None:
    ...
    # send_low_fee_tx
    _set_step(scenario_id, "send_low_fee_tx", "running", f"Sending with fee_rate={LOW_FEE_RATE} sat/vbyte…")
    try:
        w_rpc = _wallet_rpc()
        ...
        _set_step(
            ...
            technical={"txid": txid, "amount": POLICY_AMOUNT, "requested_fee_rate": LOW_FEE_RATE},
            data={"txid": txid, "destination": dest, "requested_fee_rate": LOW_FEE_RATE},
        )
    except RPCError as exc:
        _set_step(
            scenario_id, "send_low_fee_tx", "experimental",
        ...
        try:
            ...
            _set_step(
                ...
                technical={"txid": txid, "note": "fee_rate param rejected — using default fee"},
                data={"txid": txid, "destination": dest, "fallback": True},
            )
        except RPCError as exc2:
            _set_step(scenario_id, "send_low_fee_tx", "error", "sendtoaddress failed.", error=str(exc2))
            _finish_scenario(scenario_id, False)
            return
    ...
    comparison = {
        ...
        "is_below_typical": (actual_rate is not None and actual_rate <= 2.0) if actual_rate else False,
        "note": "fallback used — custom fee_rate not available" if is_fallback else "fee_rate param applied",
    }
    _set_step(
        scenario_id, "compare_fee_rate", "success",
...
def _build_proof_low_fee(scenario_id: str) -> None:
    ...
    try:
        ...
        proof = {
            ...
            "block_height": mine_data.get("block_height"),
            "success": confirm_data.get("confirmed", False),
            "unavailable_features": ["custom_fee_rate — fallback used"] if tx_data.get("fallback") else [],
            "warnings": [],
        }
        with _state_lock:
            _state["scenarios"][scenario_id]["proof"] = proof
...
def _run_rbf_replacement(scenario_id: str) -> None:
    ...
    # send_replaceable_tx
    _set_step(scenario_id, "send_replaceable_tx", "running", "Sending replaceable (BIP125 RBF) transaction…")
    try:
        w_rpc = _wallet_rpc()
        dest = w_rpc.getnewaddress("rbf_dest", "bech32")
        txid = w_rpc.sendtoaddress(dest, POLICY_AMOUNT, replaceable=True)
        _set_step(
            scenario_id, "send_replaceable_tx", "success",
            f"Replaceable tx sent — TXID: {txid}",
            technical={"txid": txid, "replaceable": True, "amount": POLICY_AMOUNT},
            data={"txid": txid, "destination": dest},
        )
    except RPCError as exc:
        _set_step(scenario_id, "send_replaceable_tx", "error", "sendtoaddress (replaceable=true) failed.", error=str(exc))
        _finish_scenario(scenario_id, False)
        return
    ...
    # bump_fee
    _set_step(scenario_id, "bump_fee", "running", "Calling bumpfee to replace transaction (RBF)…")
    try:
        w_rpc = _wallet_rpc()
        ...
        _set_step(
            scenario_id, "bump_fee", "success",
            f"RBF applied — new TXID: {new_txid}, fee bump: {orig_fee} → {new_fee} BTC",
            technical=bump_result,
            data={"new_txid": new_txid, "original_txid": txid, "new_fee": new_fee, "orig_fee": orig_fee},
        )
    except RPCError as exc:
        _set_step(
            scenario_id, "bump_fee", "experimental",
    ...
    # verify_replacement
    _set_step(scenario_id, "verify_replacement", "running", f"Verifying replacement tx {new_txid[:12] if new_txid else '?'}…")
    try:
        w_rpc = _wallet_rpc()
        ...
        _set_step(
            scenario_id, "verify_replacement", "success",
            f"Replacement in mempool — fee_rate={new_rate} sat/vb (was {original_fee_rate})",
            technical=new_entry,
            data={"new_txid": new_txid, "new_fee_rate_sat_vb": new_rate, "original_fee_rate": original_fee_rate},
        )
    except RPCError as exc:
        _set_step(scenario_id, "verify_replacement", "error", "Replacement not found in mempool.", error=str(exc))
        _finish_scenario(scenario_id, False)
        return
    ...
    # confirm_replacement
    _set_step(scenario_id, "confirm_replacement", "running", "Verifying replacement confirmed…")
    try:
        w_rpc = _wallet_rpc()
        ...
        if confirmations > 0:
            _set_step(
                scenario_id, "confirm_replacement", "success",
                f"Replacement confirmed — {confirmations} confirmation(s)",
                technical={"new_txid": new_txid, "confirmations": confirmations},
                data={"confirmed": True, "confirmations": confirmations, "new_txid": new_txid},
            )
        else:
            _set_step(scenario_id, "confirm_replacement", "error", "Replacement not confirmed.", error="confirmations=0")
            _finish_scenario(scenario_id, False)
            return
    except RPCError as exc:
        _set_step(scenario_id, "confirm_replacement", "error", "gettransaction failed for replacement.", error=str(exc))
        _finish_scenario(scenario_id, False)
        return
...
def _mine_and_confirm_rbf_fallback(scenario_id: str, txid: str, original_fee_rate: Any) -> None:
    ...
    _set_step(scenario_id, "verify_replacement", "experimental",
    ...
    try:
        ...
        confirmations = tx.get("confirmations", 0)
        _set_step(
            scenario_id, "confirm_replacement",
            "success" if confirmations > 0 else "error",
            f"Original tx confirmed — {confirmations} confirmation(s)",
            data={"confirmed": confirmations > 0, "confirmations": confirmations, "fallback": True},
        )
    except RPCError as exc:
        _set_step(scenario_id, "confirm_replacement", "error", "gettransaction failed.", error=str(exc))
...
def _build_proof_rbf_fallback(scenario_id: str, txid: str, original_fee_rate: Any) -> None:
    ...
    try:
        ...
        with _state_lock:
            _state["scenarios"][scenario_id]["proof"] = proof
        _set_step(
            scenario_id, "build_proof", "experimental",
            "Partial proof (RBF unavailable).", technical=proof, data={"proof_ready": True},
        )
    except Exception as exc:
        _set_step(scenario_id, "build_proof", "error", "Proof assembly failed.", error=str(exc))
...
def _run_cpfp_package(scenario_id: str) -> None:
    ...
    _set_step(scenario_id, "send_parent_tx", "running", f"Sending low-fee parent tx ({LOW_FEE_RATE} sat/vb)…")
    try:
        w_rpc = _wallet_rpc()
        ...
        _set_step(
            scenario_id, "send_parent_tx", "success",
            f"Parent tx sent — TXID: {parent_txid}",
            technical={"txid": parent_txid, "fee_rate_used": used_fee_rate},
            data={"parent_txid": parent_txid, "parent_dest": parent_dest, "fee_rate_used": used_fee_rate},
        )
    except RPCError as exc:
        _set_step(scenario_id, "send_parent_tx", "error", "Failed to send parent tx.", error=str(exc))
        _finish_scenario(scenario_id, False)
        return
    ...
    _set_step(scenario_id, "build_child_tx", "running",
    ...
    try:
        ...
        if not parent_utxo:
            _set_step(
                scenario_id, "build_child_tx", "experimental",
        ...
        funded = w_rpc.fundrawtransaction(raw_hex, {"fee_rate": HIGH_FEE_RATE, "subtractFeeFromOutputs": [0]})
        ...
        _set_step(
            scenario_id, "build_child_tx", "success",
    ...
    try:
        ...
        child_fee_rate = round(child_fee * 1e8 / child_vsize, 2) if child_fee and child_vsize else None
        package_fee = (parent_fee_btc or 0.0) + (child_fee or 0.0)
        package_vsize = (parent_vsize or 0) + (child_vsize or 0)
        package_rate = round(package_fee * 1e8 / package_vsize, 2) if package_fee and package_vsize else None
        _set_step(
            scenario_id, "verify_cpfp", "success",
            f"Package rate={package_rate} sat/vb (parent={parent_fee_rate}, child={child_fee_rate})",
            technical={
                "parent_txid": parent_txid, "child_txid": child_txid,
        ...
        _set_step(scenario_id, "verify_cpfp", "error", "Could not verify CPFP package.", error=str(exc))
        _finish_scenario(scenario_id, False)
        return
    ...
    try:
        ...
        _set_step(
            ...
            data={"parent_confirmed": p_conf > 0, "child_confirmed": c_conf > 0, "both_confirmed": both_confirmed},
        )
        if not both_confirmed:
            _finish_scenario(scenario_id, False)
            return
    except RPCError as exc:
        _set_step(scenario_id, "confirm_cpfp", "error", "gettransaction failed.", error=str(exc))
        _finish_scenario(scenario_id, False)
        return
...
def _mine_cpfp_fallback(scenario_id: str, parent_txid: str, parent_fee_rate: Any) -> None:
    _set_step(scenario_id, "verify_cpfp", "experimental",
    ...
    try:
        ...
        _set_step(
            ...
            data={"parent_confirmed": p_conf > 0, "child_confirmed": False, "fallback": True},
        )
    except RPCError as exc:
        _set_step(scenario_id, "confirm_cpfp", "error", "gettransaction failed.", error=str(exc))
...
def _build_proof_cpfp(scenario_id: str, parent_txid: str, child_txid: str) -> None:
    ...
    try:
        ...
        proof = {
            ...
            "parent_txid": parent_txid,
            "child_txid": child_txid,
            "parent_fee_rate_sat_vb": _get_step_data(scenario_id, "verify_parent").get("fee_rate_sat_vb"),
...
def _build_proof_cpfp_fallback(scenario_id: str, parent_txid: str, parent_fee_rate: Any) -> None:
    ...
    try:
        ...
        proof = {
            ...
            "block_height": mine_data.get("block_height"),
            "success": False,
            "unavailable_features": ["cpfp_child_tx — child construction via raw tx pipeline failed"],
        ...
        _set_step(
            scenario_id, "build_proof", "experimental",
            "Partial proof (CPFP child unavailable).", technical=proof, data={"proof_ready": True},
        )
    except Exception as exc:
        _set_step(scenario_id, "build_proof", "error", "Proof assembly failed.", error=str(exc))
